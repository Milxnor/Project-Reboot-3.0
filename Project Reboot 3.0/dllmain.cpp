    ...
    if (Fortnite_Version == 17.30)
    {
...

        {
            if (Index == 0) // Start of Event
            {
                ...
                MasterScripting->ProcessEvent(MasterScripting->FindFunction("EnableTheMutator"));
            ...
            if (Index == 2) // Slide
            {
                ...
                Script->Get<bool>(Script->GetOffset("bRaceStarted")) = true;
            ...
            if (Index == 9) // Clouds
            {
                for (int i = 0; i < ClientConnections.Num(); i++)
                {
                    ...
                    auto RemoveComponent = CurrentPawn->GetComponentByClass(FindObject<UClass>("/Buffet/Gameplay/Blueprints/Stars/BP_Buffet_Stars_PlayerComponent.BP_Buffet_Stars_PlayerComponent_C"));
...
{
public:
    struct FGameplayTag                          PredefinedCosmeticSetTag;                          // 0x0(0x8)(Edit, BlueprintVisible, NoDestructor, HasGetValueTypeHash, NativeAccessSpecifierPublic)
...
void OnRep_CosmeticLoadout(UObject* Pawn)
{
    static auto Func = FindObject<UFunction>(L"/Script/FortniteGame.FortPlayerPawn.OnRep_CosmeticLoadout");

    struct {} Parms{};

    Pawn->ProcessEvent(Func, &Parms);
}

AFortPlayerPawnAthena* (*SpawnBotOriginal)(UObject* BotManager, FVector SpawnLoc, FRotator SpawnRot, UFortAthenaAIBotCustomizationData* BotData, FFortAthenaAIBotRunTimeCustomizationData RuntimeBotData);

AFortPlayerPawnAthena* SpawnBotHook(UObject* BotManager, FVector SpawnLoc, FRotator SpawnRot, UFortAthenaAIBotCustomizationData* BotData, FFortAthenaAIBotRunTimeCustomizationData RuntimeBotData)
{
    LOG_INFO(LogDev, "RA: {:x}", __int64(_ReturnAddress()));
    LOG_INFO(LogDev, "SBR: {:x}", FindSpawnBotRet() - __int64(GetModuleHandleW(0)));
    LOG_INFO(LogDev, "data: {:x}, {:x}, {:x}, {:x}, {:x}", (uint64_t) BotManager, *(uint64_t*)&SpawnLoc, *(uint64_t*)&SpawnRot, (uint64_t) BotData, *(uint64_t*)&RuntimeBotData);
    if (__int64(_ReturnAddress()) == FindSpawnBotRet()) return SpawnBotOriginal(BotManager, SpawnLoc, SpawnRot, BotData, RuntimeBotData);
    auto CustomizationData = BotData;
    ...
    /*auto HeroDefinitionOffset = CharacterToApply->GetOffset("HeroDefinition");
    auto HeroDefinition = CharacterCustomization->Get(HeroDefinitionOffset);
    auto SpecializationsOffset = HeroDefinition->GetOffset("Specializations");
    auto& Specializations = HeroDefinition->Get<TArray<TSoftObjectPtr<UObject *>>>(SpecializationsOffset);

    static auto CosmeticLoadoutBCOffset = Controller->GetOffset("CosmeticLoadoutBC");
    Controller->Get<FFortAthenaLoadout>(CosmeticLoadoutBCOffset) = *CharacterCustomizationLoadout;
    for (int32 /*size_t* / i = 0; i < Specializations.Num(); i++)
    {
        UObject* Spec = LoadObject<UObject>(Specializations.At(i).SoftObjectPtr.ObjectID.AssetPathName.ToString());
        auto CharacterPartsOffset = Spec->GetOffset("CharacterParts");
        auto& CharacterParts = Spec->Get<TArray<TSoftObjectPtr<UObject*>>>(CharacterPartsOffset);

        if (Spec)
        {
            for (int32 /*size_t* / i = 0; i < CharacterParts.Num(); i++)
            {
                UObject* Part = LoadObject<UObject>(CharacterParts.At(i).SoftObjectPtr.ObjectID.AssetPathName.ToString());
                auto CharacterPartTypeOffset = Part->GetOffset("CharacterPartType");
                auto& CharacterPartType = Part->Get<EFortCustomPartType>(CharacterPartTypeOffset);
                Pawn->ServerChoosePart(CharacterPartType, Part);
            }
        }
    }*/

    //(*Pawn->GetCosmeticLoadout()) = *CharacterCustomizationLoadout;
    //OnRep_CosmeticLoadout(Pawn);

    //ApplyCID(Pawn, CharacterToApply, true); // bruhh
    if (BotData->GetCharacterCustomization()->GetCustomizationLoadout()->GetCharacter()->GetName() == "CID_556_Athena_Commando_F_RebirthDefaultA")
    {
        ...
        if (Tag == "Athena.Faction.Alter") {
            BotData->GetCharacterCustomization()->GetCustomizationLoadout()->GetCharacter() = LoadObject<UFortItemDefinition>("/Game/Athena/Items/Cosmetics/Characters/CID_NPC_Athena_Commando_M_HenchmanBad.CID_NPC_Athena_Commando_M_HenchmanBad");
        }
        else /*if (POI.ends_with("_Ego"))*/ if (Tag == "Athena.Faction.Ego") {
            BotData->GetCharacterCustomization()->GetCustomizationLoadout()->GetCharacter() = LoadObject<UFortItemDefinition>("/Game/Athena/Items/Cosmetics/Characters/CID_NPC_Athena_Commando_M_HenchmanGood.CID_NPC_Athena_Commando_M_HenchmanGood");
        }
        else if (Tag == "Athena.Faction.Default.POI.Box") {
            BotData->GetCharacterCustomization()->GetCustomizationLoadout()->GetCharacter() = LoadObject<UFortItemDefinition>("/Game/Athena/Items/Cosmetics/Characters/CID_NPC_Athena_Commando_M_Scrapyard.CID_NPC_Athena_Commando_M_Scrapyard");
        }
        else {
            LOG_INFO(LogDev, "GTag: {}", Tag.c_str());
        }
        //boss = false;
        //}
    }
    ...
    AActor* SpawnLocator = GetWorld()->SpawnActor<APawn>(APawn::StaticClass(), SpawnLoc, SpawnRot.Quaternion());
    if (!SpawnLocator) return nullptr;
    //static auto BotMutator = FindFirstMutator<AFortAthenaMutator_Bots>(FindObject<UClass>(L"/Script/FortniteGame.FortAthenaMutator_Bots"));
    //auto Pawn = GetWorld()->SpawnActor<AFortPlayerPawn>(PawnClass, SpawnLoc, SpawnRot.Quaternion());
    auto Pawn = ((AFortAthenaMutator_Bots *)Globals::BM)->SpawnBot(PawnClass /*PhoebePawnClass*/, SpawnLocator, SpawnLoc, SpawnRot, true) /*SpawnBotOriginal(BotManager, SpawnLoc, SpawnRot, BotData, RuntimeBotData)*/;
    ...
    Pawn->TeleportTo(SpawnLoc, SpawnRot);
    ...
    Controller->Get<FFortAthenaLoadout *>(Controller->GetOffset("CosmeticLoadoutBC")) = BotData->GetCharacterCustomization()->GetCustomizationLoadout();
    auto c = BotData->GetCharacterCustomization()->GetCustomizationLoadout()->GetCharacter();
    auto h = c->Get(c->GetOffset("HeroDefinition"));
    auto s = h->Get<TArray<TSoftObjectPtr<UObject>>>(h->GetOffset("Specializations"));
    for (int32 /*size_t*/ i = 0; i < s.Num(); i++)
    {
        UObject* Spec = LoadObject<UObject>(s.At(i).SoftObjectPtr.ObjectID.AssetPathName.ToString());

        if (Spec)
        {
            auto p = Spec->Get<TArray<TSoftObjectPtr<UObject>>>(Spec->GetOffset("CharacterParts"));
            for (int32 /*size_t*/ i = 0; i < p.Num(); i++)
            {
                UObject* Part = LoadObject<UObject>(p.At(i).SoftObjectPtr.ObjectID.AssetPathName.ToString());
                auto t = Part->Get<EFortCustomPartType>(Part->GetOffset("CharacterPartType"));
                Pawn->ServerChoosePart(t, Part);
            }
        }
    }
    *Pawn->GetCosmeticLoadout() = *BotData->GetCharacterCustomization()->GetCustomizationLoadout();
    ...
    static auto BTOffset = CustomizationData->GetOffset("BehaviorTree");
    auto BT = CustomizationData->Get(BTOffset);

    auto BotManagerSetup = (void (*)(__int64 BotManaager, __int64 Pawn, __int64 BehaviorTree, __int64 a4, DWORD * SkillLevel, __int64 idk, __int64 StartupInventory, __int64 BotNameSettings, __int64 idk_1, BYTE * CanRespawnOnDeath, unsigned __int8 BitFieldDataThing, BYTE * CustomSquadId, FFortAthenaAIBotRunTimeCustomizationData InRuntimeBotData, __int64 pl, __int64 oo, __int64 sh))(GetModuleHandleW(0) + 0x1fb6070);

    SpawnLocator->K2_DestroyActor();
    DWORD CustomSquadId = RuntimeBotData.CustomSquadId;
    BYTE TrueByte = 1;
    BYTE FalseByte = 0;
    //BotManagerSetup(__int64(BotManager), __int64(Pawn), __int64(BT), 0, &CustomSquadId, 0, __int64(StartupInventory), __int64(BotNameSettings), 0, &FalseByte, 0, &TrueByte, RuntimeBotData, __int64(StartupInventory), __int64(StartupInventory), __int64(StartupInventory));
    ...
    else
    {
        ...
        for (int i = 0; i < StartupInventoryItems.Num(); ++i)
        {
            ...
            LOG_INFO(LogDev, "Pushing back {:p}", (void *) StartupInventoryItems.at(i).Item);
        }
    }
...
DWORD WINAPI Main(LPVOID)
{
    ...
    Sleep(3000); //I added it cause it might fix the GUI not loadign issue -Boosted
    ...
    */
    ...
    Hooking::MinHook::Hook((PVOID) Addresses::SpawnBot, (PVOID)SpawnBotHook, (PVOID*)&SpawnBotOriginal);
    ...
    */
    ...
    /*Hooking::MinHook::Hook(FortPlayerControllerAthenaDefault, FindObject<UFunction>(L"/Script/Engine.PlayerController.ClientWasKicked"),
        AFortPlayerController::ClientWasKickedHook, (void**)&AFortPlayerController::ClientWasKickedOriginal, false);*/
    ...
    LPVOID ServerReadyToStartMatchAddr = (LPVOID)((FortPlayerControllerAthenaDefault->VFTable)[GetFunctionIdxOrPtr(FindObject<UFunction>("FortPlayerController.ServerReadyToStartMatch")) / 8]);
    LPVOID ServerLoadingScreenDroppedAddr = (LPVOID)((FortPlayerControllerAthenaDefault->VFTable)[GetFunctionIdxOrPtr(FindObject<UFunction>("FortPlayerController.ServerLoadingScreenDropped")) / 8]);
    ...
    Hooking::MinHook::Hook(ServerLoadingScreenDroppedAddr, AFortPlayerController::ServerLoadingScreenDroppedHook, (PVOID*)&AFortPlayerController::ServerLoadingScreenDroppedOriginal);
    Hooking::MinHook::Hook(ServerReadyToStartMatchAddr, AFortPlayerControllerAthena::ServerReadyToStartMatchHook, (PVOID*)&AFortPlayerControllerAthena::ServerReadyToStartMatchOriginal);
...

    {
        ...
        if (Fortnite_Version > 11.00 && std::floor(Fortnite_Version) < 18) // checked on 11.31, 12.41, 14.60, 15.10, 16.40, 17.30 and 18.40
            increaseOffset += 0x8;
