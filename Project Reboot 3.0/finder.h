...
static inline uint64 FindSpawnBot()
{
	auto strRef = Memcury::Scanner::FindStringRef(L"UFortServerBotManagerAthena::SpawnBot invalid spawn location", true, 0, Fortnite_Version >= 19).Get();

	LOG_INFO(LogDev, "SBStr: 0x{:x}", strRef - __int64(GetModuleHandleW(0)));

	// 488bc4
	for (int i = 0; i < 1000; i++)
	{
		if (*(uint8_t*)(strRef - i) == 0x48 && *(uint8_t*)(strRef - i + 1) == 0x8b && *(uint8_t*)(strRef - i + 2) == 0xc4)
		{
			return strRef - i;
		}
	}
...
static inline uint64 FindSpawnBotRet()
{
	auto strRef = Memcury::Scanner::FindStringRef(L"Unable to create UFortAthenaAIBotCustomizationData object. BotClass = %s", true, 0, Fortnite_Version >= 19).Get();

	LOG_INFO(LogDev, "SBRStr: 0x{:x}", strRef - __int64(GetModuleHandleW(0)));
